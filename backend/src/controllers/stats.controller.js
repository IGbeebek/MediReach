...
const statsController = {
  ...
  async adminStats(req, res, next) {
    try {
      const [
        ...
        query("SELECT COALESCE(SUM(grand_total), 0) AS total FROM orders WHERE status = 'delivered'"),
        query("SELECT COUNT(*) AS total FROM orders WHERE created_at >= CURRENT_DATE"),
        query('SELECT COUNT(*) AS total FROM medicines'),
        query("SELECT COUNT(*) AS total FROM users WHERE role = 'pharmacist' AND status = 'active'"),
        query("SELECT COUNT(*) AS total FROM prescriptions WHERE status = 'pending'"),
        ...
        query(`SELECT name, sold_count FROM medicines ORDER BY sold_count DESC LIMIT 5`),
      ...
      const orderStatusDonut = [
        { label: 'Delivered', value: totalOrders ? Math.round(((statusMap.delivered || 0) / totalOrders) * 100) : 0, color: '#4a7c59' },
        { label: 'Shipped', value: totalOrders ? Math.round(((statusMap.shipped || 0) / totalOrders) * 100) : 0, color: '#3b82f6' },
        { label: 'Packed', value: totalOrders ? Math.round(((statusMap.packed || 0) / totalOrders) * 100) : 0, color: '#f59e0b' },
        { label: 'Pending', value: totalOrders ? Math.round((((statusMap.pending || 0) + (statusMap.prescription_review || 0) + (statusMap.verified || 0)) / totalOrders) * 100) : 0, color: '#6b7280' },
      ];
      ...
      const prescriptionRate = {
        approved: totalRx ? Math.round(((rxMap.approved || 0) / totalRx) * 100) : 0,
        rejected: totalRx ? Math.round(((rxMap.rejected || 0) / totalRx) * 100) : 0,
        pending: totalRx ? Math.round(((rxMap.pending || 0) / totalRx) * 100) : 0,
        total: totalRx,
      };
  ...
  async customerStats(req, res, next) {
    try {
      ...
      const [ordersResult, prescriptionsResult, inTransitResult, spentResult, recentResult] =
        await Promise.all([
          query('SELECT COUNT(*) AS total FROM orders WHERE user_id = $1', [userId]),
          query('SELECT COUNT(*) AS total FROM prescriptions WHERE user_id = $1', [userId]),
          query("SELECT COUNT(*) AS total FROM orders WHERE user_id = $1 AND status IN ('shipped', 'packed')", [userId]),
          query("SELECT COALESCE(SUM(grand_total), 0) AS total FROM orders WHERE user_id = $1 AND status = 'delivered'", [userId]),
          query(
  ...
  async pharmacistStats(req, res, next) {
    try {
      const [rxResult, lowStockResult, ordersTodayResult, revenueTodayResult, pendingRxResult, lowStockListResult] =
        await Promise.all([
          query("SELECT COUNT(*) AS total FROM prescriptions WHERE status = 'pending'"),
          query('SELECT COUNT(*) AS total FROM medicines WHERE stock < 20'),
          query("SELECT COUNT(*) AS total FROM orders WHERE created_at >= CURRENT_DATE"),
          query("SELECT COALESCE(SUM(grand_total), 0) AS total FROM orders WHERE created_at >= CURRENT_DATE AND status = 'delivered'"),
          ...
          query('SELECT id, name, stock FROM medicines WHERE stock < 20 ORDER BY stock ASC LIMIT 5'),
        ]);
  ...
  async publicStats(req, res, next) {
    try {
      ...
      return success(res, {
        stats: [
          { value: String(parseInt(ordersResult.rows[0].total, 10)), label: 'Orders Delivered' },
          { value: String(parseInt(medicinesResult.rows[0].total, 10)), label: 'Medicines' },
