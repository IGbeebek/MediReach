...
export default function EsewaSuccessPage() {
  ...
  useEffect(() => {
    ...
    if (!data) {
      setStatus('failed');
      addToast('Missing eSewa payment data', 'error');
      return;
    }
  ...
  if (status === 'verifying') {
    return (
      <div className="max-w-md mx-auto text-center py-16 page-enter">
        ...
        <p className="text-charcoal/60 mt-2">Please wait while we confirm your eSewa payment.</p>
      </div>
    );
  }
  ...
  if (status === 'success') {
    return (
      <div className="max-w-md mx-auto text-center py-16 page-enter">
        ...
        <p className="text-charcoal/60 mt-2">Your eSewa payment has been verified. Your order is being processed.</p>
        <div className="flex gap-3 justify-center mt-6">
          <Link to="/customer/orders" className="rounded-lg bg-primary px-6 py-2.5 font-medium text-white hover:bg-primary-dark">
            View My Orders
          </Link>
          <Link to="/customer/medicines" className="rounded-lg border border-primary text-primary px-6 py-2.5 font-medium hover:bg-primary/5">
            Continue Shopping
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="max-w-md mx-auto text-center py-16 page-enter">
      ...
      <p className="text-charcoal/60 mt-2">We couldn't verify your eSewa payment. If money was deducted, it will be refunded automatically.</p>
      <div className="flex gap-3 justify-center mt-6">
        <Link to="/customer/orders" className="rounded-lg bg-primary px-6 py-2.5 font-medium text-white hover:bg-primary-dark">
          View My Orders
        </Link>
